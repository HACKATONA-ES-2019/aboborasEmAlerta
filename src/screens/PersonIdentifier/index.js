...
export const PersonIdentifier = (props) => {
    ...
    const onConfirm = async () => {
        ...
        const people = Object.entries(disaster.people).map(([key, value]) => ({...value, id: key}))
        const person = people.find(p => p.cpf === cpf)
        if (person) {
            const newValue = {
                people: {...disaster.people, [person.id]: {...person, situation: value}}
            }

            await firestore.collection('/disasters').doc(disasterId).set(newValue, {merge: true})
        }
        else {
            const newValu = {
         ...
         { !showCamera && 
            ...
            <Styles.space>
                <Row> 
                    ...
                    <Button style={{marginLeft: 30, width: 100, justifyContent: "center"}} type="primary" onClick={onConfirm}>Confirmar</Button>
                </Row>
            </Styles.space>
            </div>
         }
