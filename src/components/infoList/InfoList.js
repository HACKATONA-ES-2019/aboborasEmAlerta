  ...
  render() {
    ...
    return (
      <InfiniteScroll pageStart={0} useWindow={false}>
        <List
          header={
            <div style={{ display: 'flex', overflowX: 'auto', justifyContent:"center", marginLeft: 10, marginRight: 10 }}>
              {Object.entries(Constants.situations).map(([key, value]) => (
                <CustomTag style={{fontSize: 14, marginLeft:'2vh', marginRight:'2vh'}} checked={this.state.tags[key]} handleChange={(checked) => this.onChangeTag(key, checked)}>
